...
fn lower_statement<'a>(
    ...
    match stmt {
        ...
        oxc::Statement::ForStatement(for_stmt) => {
            ...
            let init_is_expression = matches!(
                &for_stmt.init,
                Some(init) if !matches!(
                    init,
                    oxc::ForStatementInit::VariableDeclaration(_)
                        | oxc::ForStatementInit::SequenceExpression(_)
                )
            );
            let init_block = builder.try_enter(BlockKind::Loop, |builder, _block_id| {
                let init_span = match &for_stmt.init {
                    ...
                    Some(init) => {
                        ...
                        if !init_is_expression {
                            // Sequence expressions lower through a value block. Keep their result
                            // in a declaration-shaped initializer so the block remains reachable.
